  ...
  return (
    <header className="fixed top-0 w-full bg-white/95 backdrop-blur-xl shadow-sm z-50 border-b border-orange-100/50 py-5 transition-all duration-500">
      <div className="container mx-auto max-w-[1400px] px-6 lg:px-12 flex justify-between items-center">
        {/* Logo */}
        <button onClick={() => handleNav('home')} className="flex items-center gap-2 md:gap-4 group focus:outline-none shrink-0">
        ...
        {/* Desktop Nav */}
        <div className={`hidden lg:flex items-center ${lang === 'ta' ? 'gap-4' : 'gap-12'}`}>
          <ul className={`flex items-center font-bold text-gray-500 uppercase ${lang === 'ta' ? 'gap-4 text-[11px]' : 'gap-12 text-xs tracking-widest'}`}>
            {menuItems.map((item) => (
              <li key={item.id}>
                <button 
          ...
          <div className={`flex items-center ml-4 ${lang === 'ta' ? 'gap-3' : 'gap-6'}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button 
                onClick={toggleMusic}
                className={`relative group rounded-full flex items-center justify-center transition-all duration-500 ${lang === 'ta' ? 'w-9 h-9' : 'w-11 h-11'} ${isMusicPlaying ? 'bg-primary shadow-[0_0_20px_rgba(255,107,53,0.4)]' : 'bg-gray-100 hover:bg-gray-200'}`}
              ...
              >
                ...
                {isMusicPlaying ? (
                  <svg className={`${lang === 'ta' ? 'w-4 h-4' : 'w-5 h-5'} text-white relative z-10`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>
                  </svg>
                ) : (
                  <svg className={`${lang === 'ta' ? 'w-4 h-4' : 'w-5 h-5'} text-gray-400 group-hover:text-primary relative z-10`} fill="currentColor" viewBox="0 0 24 24">
            ...
            <button 
              ...
              className={`bg-gradient-to-br from-primary to-secondary hover:shadow-xl hover:-translate-y-0.5 text-white rounded-full shadow-lg transition-all font-bold uppercase tracking-widest shrink-0 ${lang === 'ta' ? 'px-4 py-2 text-[10px]' : 'px-8 py-3 text-xs'}`}
            >
              {t.donate}
            </button>
          </div>
